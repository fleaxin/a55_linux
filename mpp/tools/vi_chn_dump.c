...
static td_s32 vi_chn_dump_make_frame_file_name(ot_vi_pipe vi_pipe, ot_vi_chn vi_chn, td_u32 frame_cnt)
{
    ...
    /* make file name */
    if (snprintf_s(yuv_name, FILE_NAME_LENGTH, FILE_NAME_LENGTH - 1, "./vi_pipe%d_chn%d_%dx%d_%s_%d.%s",
...
static td_void vi_save_raw_file(const ot_video_frame *v_buf, td_u32 byte_align, FILE *pfd)
{
    ...
    printf("saving......raw data......stride[0]: %d, width: %d, height: %d\n",
...
{
    const td_char *argv_name[4] = {"ot_vi_pipe", "ot_vi_chn", "frame_cnt", "dump_frame_pos"}; /* 4: arg nums */
...
static td_s32 vi_chn_dump_get_argv_val(char *argv[], td_s32 index, td_s32 min_val, td_s32 max_val, td_s32 *val)
{
    ...
    if ((result < min_val) || (result > max_val)) {
        printf("Failure: input arg_index(%u) arg_name(%s) arg_val (%d) is wrong. should be [%d, %d]!\n",
...
#endif
{
    ...
    if (vi_chn_dump_get_argv_val(argv, 1, 0, OT_VI_MAX_PIPE_NUM - 1, &g_vi_pipe) != TD_SUCCESS) {
        tool_usage();
        return TD_FAILURE;
    }

    if (vi_chn_dump_get_argv_val(argv, 2, 0, OT_VI_MAX_CHN_NUM - 1, &g_vi_chn) != TD_SUCCESS) { /* arg2 */
        tool_usage();
        return TD_FAILURE;
    }

    if (vi_chn_dump_get_argv_val(argv, 3, 0, MAX_DUMP_FRAME_CNT, &frame_cnt) != TD_SUCCESS) { /* arg3 */
        tool_usage();
        return TD_FAILURE;
    }

    if (argc == 5) { /* 5: arg num */
        if (vi_chn_dump_get_argv_val(argv, 4, 0, OT_ISP_DUMP_FRAME_POS_BUTT - 1, &dump_pos) != TD_SUCCESS) { /* arg4 */
            tool_usage();
            return TD_FAILURE;
        }
        g_dump_frame_pos = dump_pos;
