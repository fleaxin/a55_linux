...
static void sample_yuv_8bit_dump(const ot_video_frame *video_frame_buf, FILE *pfd)
{
    ...
    for (h = 0; h < video_frame_buf->height; h++) {
        ...
        fwrite(mem_content, video_frame_buf->width, 1, pfd);
    }
...
static td_s32 vpss_src_dump_open_yuv_file(ot_vpss_grp grp, const ot_video_frame *frame)
{
    ...
    if (snprintf_s(yuv_name, FILE_NAME_LENGTH, FILE_NAME_LENGTH - 1, "./vpss%d_%dx%d_%s.yuv",
...
#endif
{
    ...
    errno = 0;
    g_vpss_grp = (td_s32)strtol(argv[1], &end_ptr, 10); /* base 10 */
...
#ifndef __LITEOS__
    signal(SIGINT, vpss_src_dump_handle_sig);
    signal(SIGTERM, vpss_src_dump_handle_sig);
#endif
