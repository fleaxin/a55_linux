...
{
    /* make file name */
    if (snprintf_s(g_file_name, FILE_NAME_LENGTH, FILE_NAME_LENGTH - 1, "./vi_pipe%d_%dx%d_%s_%d_%d.%s",
...
#endif
{
    ...
    if (argc < 3 || argc > 4) { /* 3/4: arg num */
        tool_usage();
        return TD_FAILURE;
    }

    if (vi_pipe_dump_get_argv_val(argv, 1, 0, OT_VI_MAX_PIPE_NUM - 1, &g_vi_pipe) != TD_SUCCESS) {
        tool_usage();
        return TD_FAILURE;
    }

    if (vi_pipe_dump_get_argv_val(argv, 2, 0, MAX_DUMP_FRAME_CNT, &frame_cnt) != TD_SUCCESS) { /* arg2 */
        tool_usage();
        return TD_FAILURE;
    }

    if (argc > 3 && (vi_pipe_dump_get_argv_val(argv, 3, 0, 1, &byte_align) != TD_SUCCESS)) { /* arg3 */
        tool_usage();
        return TD_FAILURE;
    }
