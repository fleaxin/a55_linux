...
#define VPSS_GET_CHN_FRAME_NUM 40
...
static td_s32 vpss_chn_dump_noncontinuous_frame(ot_vpss_grp grp, ot_vpss_chn chn, td_u32 frame_cnt, td_bool send_to_vgs)
{
...
static td_s32 vpss_chn_dump_get_frame(ot_vpss_grp grp, ot_vpss_chn chn, td_u32 frame_cnt)
{
    ...
    td_bool get_continuous_frame = vpss_chn_dump_is_frame_continuous(frame_cnt, send_to_vgs);
    if (get_continuous_frame == TD_TRUE) {
        ret = vpss_chn_dump_continuous_frame(grp, chn, frame_cnt);
        if (ret != TD_SUCCESS) {
            vpss_chn_dump_release_multi_frame(grp, chn, frame_cnt);
        }
    } else {
        ret = vpss_chn_dump_noncontinuous_frame(grp, chn, frame_cnt, send_to_vgs);
    }
...
static td_void vpss_chn_dump_init_global_params()
{
    g_vpss_depth_flag = 0;
    g_signal_flag = 0;
    g_user_page_addr[0] = TD_NULL;
...
#endif
{
    ...
    if (((argc > 1) && (strncmp(argv[1], "-h", 2) == 0)) || (argc != 4)) { /* 2: help, 4: args */
        goto print_usage;
    }

    errno = 0;
    g_vpss_grp = (td_s32)strtol(argv[1], &end_ptr, base);
    ...
    errno = 0;
    g_vpss_chn = (td_s32)strtol(argv[2], &end_ptr, base); /* 2: chn id */
    if ((errno != 0) || (*end_ptr != '\0')) {
        goto print_usage;
    }
...
#ifndef __LITEOS__
    signal(SIGINT, vpss_chn_dump_handle_sig);
    signal(SIGTERM, vpss_chn_dump_handle_sig);
#endif
