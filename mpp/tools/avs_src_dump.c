...
static ot_avs_pipe g_avs_pipe = 0;
...
static td_void avs_pipe_dump_covert_chroma_sp42x_to_planar(const ot_video_frame *frame, FILE *fd,
...
static td_void avs_yuv_8bit_dump(const ot_video_frame *frame, FILE *fd)
{
    ...
    if (pixel_format != OT_PIXEL_FORMAT_YUV_400) {
        avs_pipe_dump_covert_chroma_sp42x_to_planar(frame, fd, uv_height, is_uv_invert);
    }
...
static td_void avs_restore(ot_avs_grp avs_grp, ot_avs_pipe avs_pipe)
{
    td_s32 ret;

    if (g_frame.pool_id != OT_VB_INVALID_POOL_ID) {
        ret = ss_mpi_avs_release_pipe_frame(avs_grp, avs_pipe, &g_frame);
        if (ret != TD_SUCCESS) {
...
void avs_src_dump_handle_sig(int signo)
{
    if (g_signal_flag) {
        return;
    }
...
td_s32 avs_pipe_dump_make_yuv_file_name(ot_avs_grp grp, ot_avs_pipe pipe, td_u32 frame_cnt)
{
    ...
    /* make file name */
    if (g_frame.video_frame.video_format == OT_VIDEO_FORMAT_TILE_16x8) {
        if (snprintf_s(yuv_name, FILE_NAME_LENGTH, FILE_NAME_LENGTH - 1, "./avs_grp%d_pipe%d_%ux%u_s%u_%s_%u.blk",
            grp, pipe, g_frame.video_frame.width, g_frame.video_frame.height, g_frame.video_frame.stride[0],
            pixel_str, frame_cnt) == -1) {
            return TD_FAILURE;
        }
    } else {
        if (snprintf_s(yuv_name, FILE_NAME_LENGTH, FILE_NAME_LENGTH - 1, "./avs_grp%d_pipe%d_%ux%u_%s_%u.yuv",
            grp, pipe, g_frame.video_frame.width, g_frame.video_frame.height, pixel_str, frame_cnt) == -1) {
            return TD_FAILURE;
        }
    }
    printf("Dump YUV frame of avs pipe %d  to file: \"%s\"\n", pipe, yuv_name);
...
td_void avs_pipe_dump_set_vgs_frame_info(ot_video_frame_info *vgs_frame_info, const avs_dump_mem *dump_mem,
...
td_s32 avs_pipe_dump_init_vgs_pool(avs_dump_mem *dump_mem, ot_vb_calc_cfg *vb_calc_cfg)
{
...
td_s32 avs_pipe_dump_send_vgs_and_dump(const ot_video_frame_info *avs_frame_info)
{
    ot_video_frame_info vgs_frame_info = {0};
    ot_vgs_task_attr vgs_task_attr;
    ot_vb_calc_cfg vb_calc_cfg = {0};

    if (avs_pipe_dump_init_vgs_pool(&g_dump_mem, &vb_calc_cfg) != TD_SUCCESS) {
        printf("init vgs pool failed\n");
        return TD_FAILURE;
    }

    avs_pipe_dump_set_vgs_frame_info(&vgs_frame_info, &g_dump_mem, &vb_calc_cfg, avs_frame_info);
...
static void avs_blk_yuv_dump(const ot_video_frame* v_buf, FILE* pfd)
{
    unsigned int h;
    char* virt_y = TD_NULL;
    char* mem_content = TD_NULL;
    ...
    g_size = (v_buf->stride[0]) * (v_buf->height);
    ...
    virt_y = g_user_page_addr[0];

    fprintf(stderr, "saving......blk......");
    fflush(stderr);

    for (h = 0; h < v_buf->height; h++) {
        mem_content = virt_y + h * v_buf->stride[0];
        fwrite(mem_content, v_buf->stride[0], 1, pfd);
    }
...
static td_s32 avs_linear_yuv_dump(ot_avs_grp grp, ot_avs_pipe pipe)
{
    ...
    if (is_send_to_vgs) {
        if (avs_pipe_dump_send_vgs_and_dump(&g_frame) != TD_SUCCESS) {
            ss_mpi_avs_release_pipe_frame(grp, pipe, &g_frame);
            g_frame.pool_id = OT_VB_INVALID_POOL_ID;
            return TD_FAILURE;
        }
    } else {
...
static td_s32 avs_pipe_dump_get_frame(ot_avs_grp grp, ot_avs_pipe pipe, td_u32 frame_cnt)
{
    td_s32 ret;
    ...
    while (cnt--) {
        ...
        if (ss_mpi_avs_get_pipe_frame(grp, pipe, &g_frame, milli_sec) != TD_SUCCESS) {
        ...
        if (g_frame.video_frame.video_format == OT_VIDEO_FORMAT_TILE_16x8) {
            avs_blk_yuv_dump(&g_frame.video_frame, g_pfd);
        } else {
            if (avs_linear_yuv_dump(grp, pipe) != TD_SUCCESS) {
                return TD_FAILURE;
            }
        }
        printf("Get frame %d!!\n", cnt);
        /* release frame after using */
        ret = ss_mpi_avs_release_pipe_frame(grp, pipe, &g_frame);
        if (ret != TD_SUCCESS) {
...
static td_s32 avs_pipe_dump_try_get_frame(ot_avs_grp grp, ot_avs_pipe pipe, td_u32 frame_cnt)
{
    td_s32 milli_sec = GET_CHN_FRAME_TIMEOUT;
    td_s32 try_times = 10;
    td_s32 ret;
    while (ss_mpi_avs_get_pipe_frame(grp, pipe, &g_frame, milli_sec) != TD_SUCCESS) {
        if (g_signal_flag == 1) {
            printf("\033[0;31mprogram termination abnormally!\033[0;39m\n");
            return TD_FAILURE;
        }
    ...
    if (avs_pipe_dump_make_yuv_file_name(grp, pipe, frame_cnt) != TD_SUCCESS) {
        ss_mpi_avs_release_pipe_frame(grp, pipe, &g_frame);
        g_frame.pool_id = OT_VB_INVALID_POOL_ID;
        return TD_FAILURE;
    }
...
td_void sample_misc_avs_src_dump(ot_avs_grp grp, ot_avs_pipe pipe, td_u32 frame_cnt)
{
    ...
    if (avs_pipe_dump_try_get_frame(grp, pipe, frame_cnt) != TD_SUCCESS) {
        goto exit;
    }

    if (avs_pipe_dump_get_frame(grp, pipe, frame_cnt) != TD_SUCCESS) {
        goto exit;
    }

exit:
    avs_restore(grp, pipe);
    return;
}
...
static td_void usage(td_void)
{
    printf("\n"
           "*************************************************\n"
           "Usage: ./avs_src_dump [grp] [pipe] [frm_cnt]\n"
           "1)grp: \n"
           "   avs group id\n"
           "2)pipe: \n"
           "   avs pipe id\n"
           ...
           "   e.g : ./avs_src_dump 1 0 2\n"
           "*************************************************\n"
           "\n");
}

static td_void avs_src_dump_init_global_params()
{
    g_signal_flag = 0;
    g_user_page_addr[0] = TD_NULL;
    g_user_page_addr[1] = TD_NULL;
...
#ifdef __LITEOS__
td_s32 avs_src_dump(int argc, char *argv[])
#else
...
#endif
{
    ...
    g_avs_pipe = (td_s32)strtol(argv[2], &end_ptr, base); /* 2: pipe id */
    if ((errno != 0) || (*end_ptr != '\0')) {
        goto print_usage;
    }

    if (!value_between(g_avs_pipe, 0, OT_AVS_PIPE_NUM - 1)) {
    ...
    avs_src_dump_init_global_params();
#ifndef __LITEOS__
    signal(SIGINT, avs_src_dump_handle_sig);
    signal(SIGTERM, avs_src_dump_handle_sig);
#endif
    sample_misc_avs_src_dump(g_avs_grp, g_avs_pipe, (td_u32)frame_cnt);
    return TD_SUCCESS;
