...
static int atoul(char *str, unsigned int *pul_value)
{
    ...
    while (*str) {
        ...
        if (isdigit(ch)) {
             ...
             * so, X := 429496729
             */
            if ((ul_result < 429496729) || ((ul_result == 429496729) && (*str < '6'))) { /* calc result : 429496729 */
                ul_result = ul_result * 10 + (*str) - 48; /* using function: 10*x + (str) - 48 */
            } else {
                *pul_value = ul_result;
                return -1;
            }
    ...
    while (*str) {
        ...
        if (isdigit(ch) || ((ch >= 'A') && (ch <= 'F'))) {
            if (ul_result < 0x10000000) {
                ul_result = (ul_result << 4) + ((ch <= '9') ? (asc2num(ch)) : (hexasc2num(ch))); /* pointer shift  4 */
            } else {
                *pul_value = ul_result;
                return -1;
            }
