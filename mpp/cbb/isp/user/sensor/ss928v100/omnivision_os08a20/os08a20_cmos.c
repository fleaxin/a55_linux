 ...
 * global variables                                                            *
 ****************************************************************************/
#define os08a20_sensor_set_ctx(dev, ctx)   ((g_os08a20_sns_state[dev]) = (ctx))
#define os08a20_sensor_reset_ctx(dev)      (g_os08a20_sns_state[dev] = TD_NULL)
#define os08a20_set_sensor_acc(dst, static_val, src) (dst = (static_val > 0) ? static_val : src)
...
static td_u32 g_init_ae_comp[OT_ISP_MAX_PIPE_NUM] = {0};
static td_float g_init_int_time_acc[OT_ISP_MAX_PIPE_NUM]  = {0};
static td_float g_init_again_acc[OT_ISP_MAX_PIPE_NUM]  = {0};
static td_float g_init_dgain_acc[OT_ISP_MAX_PIPE_NUM]  = {0};
...
#define os08a20_err_mode_print(sensor_image_mode, sns_state) \
do { \
    ...
    const ot_isp_sns_state *sns_state)
{
    ...
    os08a20_set_sensor_acc(ae_sns_dft->int_time_accu.accuracy, g_init_int_time_acc[vi_pipe], 1);

    ae_sns_dft->again_accu.accu_type = OT_ISP_AE_ACCURACY_TABLE;
    os08a20_set_sensor_acc(ae_sns_dft->again_accu.accuracy, g_init_again_acc[vi_pipe], 1);

    ae_sns_dft->dgain_accu.accu_type = OT_ISP_AE_ACCURACY_LINEAR;
    os08a20_set_sensor_acc(ae_sns_dft->dgain_accu.accuracy, g_init_dgain_acc[vi_pipe], 0.0078125);

    ae_sns_dft->isp_dgain_shift = 8; /* accuracy: 8 */
    ae_sns_dft->min_isp_dgain_target = 1 << ae_sns_dft->isp_dgain_shift;
    ae_sns_dft->max_isp_dgain_target = 4 << ae_sns_dft->isp_dgain_shift; /* max 4 */
    ...
    const ot_isp_sns_state *sns_state)
{
    ...
    ae_sns_dft->ae_compensation = g_init_ae_comp[vi_pipe] ? g_init_ae_comp[vi_pipe] : 0x40;
    ...
    const ot_isp_sns_state *sns_state)
{
    ...
    if (g_fswdr_mode[vi_pipe] == OT_ISP_FSWDR_LONG_FRAME_MODE) {
        ae_sns_dft->ae_compensation = g_init_ae_comp[vi_pipe] ? g_init_ae_comp[vi_pipe] : 56; /* ae_comp 56 */
        ae_sns_dft->ae_exp_mode = OT_ISP_AE_EXP_HIGHLIGHT_PRIOR;
    } else {
        ae_sns_dft->ae_compensation = g_init_ae_comp[vi_pipe] ? g_init_ae_comp[vi_pipe] : 32; /* ae_comp 32 */
...
static td_s32 cmos_get_isp_default(ot_vi_pipe vi_pipe, ot_isp_cmos_default *isp_def)
{
    ...
    isp_def->sensor_mode.sensor_id = OS08A20_ID;
...
static td_s32 cmos_set_image_mode(ot_vi_pipe vi_pipe, const ot_isp_cmos_sensor_image_mode *sensor_image_mode)
{
    ...
    for (i = 0; i < OS08A20_MODE_BUTT; i++) {
        if (sensor_image_mode->fps <= g_os08a20_mode_tbl[i].max_fps &&
            sensor_image_mode->width <= g_os08a20_mode_tbl[i].width &&
            sensor_image_mode->height <= g_os08a20_mode_tbl[i].height &&
    ...
    if (i >= OS08A20_MODE_BUTT) {
        os08a20_err_mode_print(sensor_image_mode, sns_state);
        return TD_FAILURE;
    }
...
static td_s32 cmos_init_sensor_exp_function(ot_isp_sensor_exp_func *sensor_exp_func)
{
    ...
    sensor_exp_func->pfn_cmos_sensor_init         = os08a20_init;
    sensor_exp_func->pfn_cmos_sensor_exit         = os08a20_exit;
...
static td_s32 sensor_register_callback(ot_vi_pipe vi_pipe, ot_isp_3a_alg_lib *ae_lib, ot_isp_3a_alg_lib *awb_lib)
{
    ...
    sns_attr_info.sensor_id = OS08A20_ID;
...
static td_s32 sensor_set_init(ot_vi_pipe vi_pipe, ot_isp_init_attr *init_attr)
{
    ...
    g_init_ae_comp[vi_pipe]  = init_attr->ae_comp;
    g_init_exposure[vi_pipe]  = init_attr->exposure;
    g_init_int_time[vi_pipe]  = init_attr->exp_time;
    g_init_int_time_acc[vi_pipe]  = init_attr->int_time_accu;
    g_init_again[vi_pipe]     = init_attr->a_gain;
    g_init_again_acc[vi_pipe]  = init_attr->again_accu;
    g_init_dgain[vi_pipe]     = init_attr->d_gain;
    g_init_dgain_acc[vi_pipe]  = init_attr->dgain_accu;
