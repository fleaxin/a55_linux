...
typedef struct {
    ...
    ot_isp_sensor_register  sns_register;
    ...
    ot_isp_cmos_sensor_image_mode sns_image_mode;
} isp_sensor_ctx;
...
#define sensor_reset_ctx(dev)    (g_sensor_ctx[dev] = TD_NULL)
                               ...
                               const ot_isp_sensor_register *sns_register)
{
    ...
    (td_void)memcpy_s(&sensor_ctx->sns_register, sizeof(ot_isp_sensor_register),
                      sns_register, sizeof(ot_isp_sensor_register));

    if (sensor_ctx->sns_register.sns_exp.pfn_cmos_sensor_global_init != TD_NULL) {
...
{
    isp_sensor_ctx *sensor_ctx = TD_NULL;
    ot_isp_sensor_exp_func *sns_exp = TD_NULL;
...
td_s32 isp_sensor_get_id(ot_vi_pipe vi_pipe, ot_sensor_id *sensor_id)
{
    isp_sensor_ctx *sensor_ctx = TD_NULL;
...
td_s32 isp_sensor_get_max_resolution(ot_vi_pipe vi_pipe, ot_isp_cmos_sensor_max_resolution **sns_max_resolution)
{
    isp_sensor_ctx *sensor_ctx = TD_NULL;
...
td_s32 isp_sensor_init(ot_vi_pipe vi_pipe)
{
    ...
    /* if I2C or SSP Dev is -1, don't init sensor */
    {
        ...
        if ((sns_regs_info->sns_type == OT_ISP_SNS_I2C_TYPE) &&
            (sns_regs_info->com_bus.i2c_dev == -1)) {
            return TD_SUCCESS;
        }

        if ((sns_regs_info->sns_type == OT_ISP_SNS_SSP_TYPE) &&
            (ssp_dev == -1)) {
            return TD_SUCCESS;
        }
    }

    if (sensor_ctx->sns_register.sns_exp.pfn_cmos_sensor_init != TD_NULL) {
...
td_void isp_sensor_exit(ot_vi_pipe vi_pipe)
{
    ...
    if (sensor_ctx->sns_register.sns_exp.pfn_cmos_sensor_exit != TD_NULL) {
...
td_s32 isp_sensor_set_image_mode(ot_vi_pipe vi_pipe, ot_isp_cmos_sensor_image_mode *sns_image_mode)
{
    isp_sensor_ctx *sensor_ctx = TD_NULL;
    ot_isp_sensor_exp_func *sns_exp = TD_NULL;
...
td_s32 isp_sensor_get_awb_gain(ot_vi_pipe vi_pipe, td_u32 *sensor_awb_gain)
{
    isp_sensor_ctx *sensor_ctx = TD_NULL;
