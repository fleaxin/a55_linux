...
td_s32 isp_mem_init_update_ctx(ot_vi_pipe vi_pipe, isp_usr_ctx *isp_ctx)
{
    ...
    if (ioctl(isp_get_fd(vi_pipe), ISP_STAGGER_ATTR_GET, &isp_ctx->stagger_attr) != TD_SUCCESS) {
...
static td_void isp_read_ext_regs(ot_vi_pipe vi_pipe)
{
    isp_usr_ctx *isp_ctx = TD_NULL;
...
static td_s32 isp_init_sensor_update(ot_vi_pipe vi_pipe, isp_usr_ctx *isp_ctx)
{
    ...
    isp_dng_ext_regs_initialize(vi_pipe, (ot_isp_dng_color_param *)(&sns_dft->dng_color_param));
    ot_ext_system_dng_static_info_valid_write(vi_pipe, sns_dft->sensor_mode.valid_dng_raw_format);

    if (sns_dft->sensor_mode.valid_dng_raw_format == TD_TRUE) {
        (td_void)memcpy_s(&isp_ctx->dng_info_ctrl.isp_dng->dng_raw_format, sizeof(ot_isp_dng_raw_format),
                          &sns_dft->sensor_mode.dng_raw_format, sizeof(ot_isp_dng_raw_format));
    } else {
...
static td_void isp_register_algs(ot_vi_pipe vi_pipe, isp_usr_ctx *isp_ctx)
{
    ...
    } else if ((isp_ctrl_param.alg_run_select == OT_ISP_ALG_RUN_FE_ONLY) && (is_virt_pipe(vi_pipe) == TD_FALSE)) {
...
{
    ot_isp_cmos_sensor_image_mode  isp_image_mode;
...
td_s32 isp_run(ot_vi_pipe vi_pipe)
{
    ...
    isp_dbg_run_end(vi_pipe, &isp_ctx->isp_dbg, isp_ctx->frame_cnt);
...
td_s32 isp_exit(ot_vi_pipe vi_pipe)
{
    ...
    vreg_munmap(isp_pre_be_reg_base(vi_pipe), VI_ISP_BE_REG_SIZE);
    vreg_munmap(isp_post_be_reg_base(vi_pipe), VI_ISP_BE_REG_SIZE);
