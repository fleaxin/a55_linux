...
static td_void isp_awb_cmm_config_get(ot_vi_pipe vi_pipe, isp_usr_ctx *isp_ctx, awb_ccm_config *awb_ccm_cfg)
{
#ifdef CONFIG_OT_SNAP_SUPPORT
    ot_vi_pipe awb_dev = (vi_pipe ? 0 : 1);

    switch (isp_ctx->linkage.snap_type) {
        default:

        case OT_SNAP_TYPE_NORM:
        case OT_SNAP_TYPE_PRO:
            if ((isp_ctx->linkage.snap_pipe_mode == ISP_SNAP_PICTURE) && (isp_ctx->linkage.load_ccm == TD_TRUE)) {
                isp_algs_ctrl(isp_ctx->algs, awb_dev, OT_ISP_AWB_CCM_CONFIG_GET, (td_void *)awb_ccm_cfg);
            } else {
                isp_algs_ctrl(isp_ctx->algs, vi_pipe, OT_ISP_AWB_CCM_CONFIG_GET, (td_void *)awb_ccm_cfg);
            }
            break;
    }
#else
...
static td_void isp_dng_color_param_update(ot_vi_pipe vi_pipe, isp_usr_ctx *isp_ctx)
{
    ...
    (td_void)memcpy_s(&dng_ccm.low_ccm, sizeof(td_u16) * OT_ISP_CCM_MATRIX_SIZE,
                      &awb_ccm_cfg.low_ccm, sizeof(td_u16) * OT_ISP_CCM_MATRIX_SIZE);
    (td_void)memcpy_s(&dng_ccm.high_ccm, sizeof(td_u16) * OT_ISP_CCM_MATRIX_SIZE,
                      &awb_ccm_cfg.high_ccm, sizeof(td_u16) * OT_ISP_CCM_MATRIX_SIZE);
