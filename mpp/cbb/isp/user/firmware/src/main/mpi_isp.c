    ...
    isp_usr_ctx *isp_ctx_info)
{
    ...
    if ((pub_attr->wnd_rect.width < OT_ISP_WIDTH_MIN) || (pub_attr->wnd_rect.width > ot_isp_res_width_max(vi_pipe)) ||
    ...
    if ((pub_attr->wnd_rect.height < OT_ISP_HEIGHT_MIN) ||
        (pub_attr->wnd_rect.height > ot_isp_res_height_max(vi_pipe)) ||
        (pub_attr->wnd_rect.height % OT_ISP_ALIGN_HEIGHT) != 0) {
        isp_err_trace("Invalid image height:%d!\n", pub_attr->wnd_rect.height);
        return OT_ERR_ISP_ILLEGAL_PARAM;
    }

    if ((pub_attr->wnd_rect.x < 0) || (pub_attr->wnd_rect.x > ot_isp_res_width_max(vi_pipe) - OT_ISP_WIDTH_MIN)) {
        isp_err_trace("Invalid image X:%d!\n", pub_attr->wnd_rect.x);
        return OT_ERR_ISP_ILLEGAL_PARAM;
    }

    if ((pub_attr->wnd_rect.y < 0) || (pub_attr->wnd_rect.y > ot_isp_res_height_max(vi_pipe) - OT_ISP_HEIGHT_MIN)) {
        isp_err_trace("Invalid image Y:%d!\n", pub_attr->wnd_rect.y);
        return OT_ERR_ISP_ILLEGAL_PARAM;
    }

    if (vi_pipe < OT_ISP_MAX_PHY_PIPE_NUM) {
        /* Get pipe size */
...
static td_s32 isp_mipi_wnd_rect_attr_check(ot_vi_pipe vi_pipe, const ot_isp_pub_attr *pub_attr)
{
...
static td_s32 isp_pub_attr_check(ot_vi_pipe vi_pipe, const ot_isp_pub_attr *pub_attr, isp_usr_ctx *isp_ctx_info)
{
    ...
    isp_check_bool_return(pub_attr->sensor_flip_en);
    isp_check_bool_return(pub_attr->sensor_mirror_en);
    isp_check_bool_return(pub_attr->mipi_crop_attr.mipi_crop_en);

    if (pub_attr->mipi_crop_attr.mipi_crop_en == TD_TRUE) {
        ret = isp_mipi_wnd_rect_attr_check(vi_pipe, pub_attr);
        if (ret != TD_SUCCESS) {
            return ret;
        }
    }
...
static td_void isp_pub_attr_write(ot_vi_pipe vi_pipe, const ot_isp_pub_attr *pub_attr, isp_usr_ctx *isp_ctx_info)
{
    ...
    ot_sensor_flip_enable_write(vi_pipe, pub_attr->sensor_flip_en);
    ot_sensor_mirror_enable_write(vi_pipe, pub_attr->sensor_mirror_en);
...
td_s32 ot_mpi_isp_get_pub_attr(ot_vi_pipe vi_pipe, ot_isp_pub_attr *pub_attr)
{
    ...
    pub_attr->sensor_flip_en   = ot_sensor_flip_enable_read(vi_pipe);
    pub_attr->sensor_mirror_en = ot_sensor_mirror_enable_read(vi_pipe);
...
td_s32 ot_mpi_isp_set_module_ctrl(ot_vi_pipe vi_pipe, const ot_isp_module_ctrl *mod_ctrl)
{
    ...
    ot_ext_system_local_cac_enable_write(vi_pipe, !((info >> 17) & 0x1));       /* bit17: local cac */
    ot_ext_system_acac_enable_write(vi_pipe, !((info >> 18) & 0x1));            /* bit18: global cac */
    ...
    ot_ext_system_agamma_enable_write(vi_pipe, !((info >> 31) & 0x1));          /* bit31: agamma */
    ot_ext_system_adgamma_enable_write(vi_pipe, !((info >> 32) & 0x1));         /* bit32: adgamma */
    ot_ext_system_crb_en_write(vi_pipe, !((info >> 33) & 0x1));                 /* bit33: crb */
...
td_s32 ot_mpi_isp_get_module_ctrl(ot_vi_pipe vi_pipe, ot_isp_module_ctrl *mod_ctrl)
{
    ...
    mod_ctrl->bit_bypass_local_cac         = !(ot_ext_system_local_cac_enable_read(vi_pipe));
    mod_ctrl->bit_bypass_acac              = !(ot_ext_system_acac_enable_read(vi_pipe));
    ...
    mod_ctrl->bit_bypass_agamma            = !(ot_ext_system_agamma_enable_read(vi_pipe));
    mod_ctrl->bit_bypass_adgamma           = !(ot_ext_system_adgamma_enable_read(vi_pipe));
    mod_ctrl->bit_bypass_crb               = !(ot_ext_system_crb_en_read(vi_pipe));
...
td_s32 ot_mpi_isp_set_drc_attr(ot_vi_pipe vi_pipe, const ot_isp_drc_attr *drc_attr)
{
    td_s32 ret;
    isp_check_pipe_return(vi_pipe);
    isp_check_pointer_return(drc_attr);
    isp_check_open_return(vi_pipe);
    isp_check_mem_init_return(vi_pipe);
...
td_s32 ot_mpi_isp_set_dehaze_attr(ot_vi_pipe vi_pipe, const ot_isp_dehaze_attr *dehaze_attr)
{
    ...
    ot_ext_system_dehaze_enable_write(vi_pipe, dehaze_attr->en);
...
td_s32 ot_mpi_isp_get_dehaze_attr(ot_vi_pipe vi_pipe, ot_isp_dehaze_attr *dehaze_attr)
{
    ...
    dehaze_attr->en               = ot_ext_system_dehaze_enable_read(vi_pipe);
...
static td_void isp_get_fe_ae_loc_statistics(td_u32 pipe_num, ot_isp_ae_stats *ae_stat, isp_stat *act_stat)
{
    ...
    size = OT_ISP_WDR_MAX_FRAME_NUM * OT_ISP_AE_ZONE_ROW * OT_ISP_AE_ZONE_COLUMN * OT_ISP_BAYER_CHN_NUM;
...
td_s32 ot_mpi_isp_get_ae_stats(ot_vi_pipe vi_pipe, ot_isp_ae_stats *ae_stat)
{
    ...
    stat_key.key = ((td_u64)ot_ext_system_statistics_ctrl_highbit_read(vi_pipe) << 32) + /* left shift 32bits */
        ot_ext_system_statistics_ctrl_lowbit_read(vi_pipe);
...
static td_s32 isp_set_wb_stats_config_check_para(const ot_isp_wb_stats_cfg *wb_cfg)
{
                                      ...
                                      isp_working_mode *isp_work_mode)
{
    ...
    ret = isp_set_wb_stats_config_check_para(wb_cfg);
    if (ret != TD_SUCCESS) {
        return ret;
    }
    ...
    ot_ext_system_awb_crop_en_write(vi_pipe, wb_cfg->crop.en);
...
static td_s32 isp_ae_crop_config_check(ot_vi_pipe vi_pipe, const ot_isp_ae_crop *crop, td_u16 width, td_u16 height)
{
    isp_check_bool_return(crop->en);
...
static td_s32 isp_ae_stats_crop_config_check(ot_vi_pipe vi_pipe, const ot_isp_ae_stats_cfg *ae_cfg)
{
    ...
    width  = ot_ext_system_be_total_width_read(vi_pipe);
    height = ot_ext_system_be_total_height_read(vi_pipe);
    ret = isp_ae_crop_config_check(vi_pipe, &ae_cfg->crop, width, height);
    if (ret != TD_SUCCESS) {
    ...
    ret = isp_ae_crop_config_check(vi_pipe, &ae_cfg->fe_crop, width, height);
    if (ret != TD_SUCCESS) {
...
static td_s32 isp_ae_stats_config_check(ot_vi_pipe vi_pipe, const ot_isp_ae_stats_cfg *ae_cfg)
{
    ...
    ret = isp_ae_stats_crop_config_check(vi_pipe, ae_cfg);
    if (ret != TD_SUCCESS) {
        return ret;
    }
...
static td_s32 isp_set_ae_stats_config(ot_vi_pipe vi_pipe, const ot_isp_ae_stats_cfg *ae_cfg)
{
    td_s32 ret;
    td_s32 i, j;
    ret = isp_ae_stats_config_check(vi_pipe, ae_cfg);
    if (ret != TD_SUCCESS) {
        return ret;
    }
    ...
    ot_ext_system_ae_crop_en_write(vi_pipe, ae_cfg->crop.en);
    ...
    ot_ext_system_ae_fe_crop_en_write(vi_pipe, ae_cfg->fe_crop.en);
    ...
    for (i = 0; i < OT_ISP_AE_ZONE_ROW; i++) {
        for (j = 0; j < OT_ISP_AE_ZONE_COLUMN; j++) {
            ot_ext_system_ae_weight_table_write(vi_pipe, (i * OT_ISP_AE_ZONE_COLUMN + j), ae_cfg->weight[i][j]);
        }
    }
...
#ifdef CONFIG_OT_ISP_AF_SUPPORT
static td_s32 isp_af_crop_config_check(ot_vi_pipe vi_pipe, const ot_isp_af_crop *crop, td_u16 width, td_u16 height)
{
    isp_check_bool_return(crop->en);
...
static td_s32 isp_af_stats_crop_config_check(ot_vi_pipe vi_pipe, const ot_isp_focus_stats_cfg *af_cfg)
{
    ...
    width  = ot_ext_system_be_total_width_read(vi_pipe);
    height = ot_ext_system_be_total_height_read(vi_pipe);
    ret = isp_af_crop_config_check(vi_pipe, &af_cfg->config.crop, width, height);
    if (ret != TD_SUCCESS) {
    ...
    width  = ot_ext_sync_total_width_read(vi_pipe);
    height = ot_ext_sync_total_height_read(vi_pipe);
    ret = isp_af_crop_config_check(vi_pipe, &af_cfg->config.fe_crop, width, height);
    if (ret != TD_SUCCESS) {
                                        ...
                                        isp_working_mode *isp_work_mode)
{
    ...
    if (af_cfg->config.zone_col < isp_work_mode->block_num) {
        isp_err_trace("the value of AF zone_col should be >= block_num:%d!\n", isp_work_mode->block_num);
        return OT_ERR_ISP_ILLEGAL_PARAM;
    }
    ...
    ret = isp_af_stats_crop_config_check(vi_pipe, af_cfg);
    if (ret != TD_SUCCESS) {
        return ret;
    }
...
static td_s32 isp_af_stats_h_param_check(const ot_isp_af_h_param *h_param, const char *name)
{
    ...
    isp_max_param_check_return(h_param->level_depend.en, 0x1, "Err %s.ld.ld_en!\n", name);
...
static td_s32 isp_af_stats_v_param_check(const ot_isp_af_v_param *v_param, const char *name)
{
    ...
    for (i = 0; i < OT_ISP_FIR_GAIN_NUM; i++) {
        isp_param_check_return(v_param->fir_gain[i], -31, 31, "Err %s.fir_gain[%d]\n!", name, i);  /* [-31, 31] */
    }
    isp_max_param_check_return(v_param->level_depend.en, 0x1, "Err %s.level_depend.ld_en!\n", name);
...
static td_s32 isp_af_stats_fv_param_check(const ot_isp_af_fv_param *fv_param)
{
    ...
    for (i = 0; i < OT_ISP_ACC_SHIFT_H_NUM; i++) {
        isp_max_param_check_return(fv_param->acc_shift_h[i], 0xF, "Err fv_param.acc_shift_h[%d]!\n", i);
    }

    for (i = 0; i < OT_ISP_ACC_SHIFT_V_NUM; i++) {
        isp_max_param_check_return(fv_param->acc_shift_v[i], 0xF, "Err fv_param.acc_shift_v[%d]!\n", i);
    }
...
td_s32 ot_mpi_isp_set_stats_cfg(ot_vi_pipe vi_pipe, const ot_isp_stats_cfg *stat_cfg)
{
    ...
    if (isp_work_mode.data_input_mode == ISP_MODE_RAW) {
        ...
        /* AE */
        ret = isp_set_ae_stats_config(vi_pipe, &stat_cfg->ae_cfg);
        if (ret != TD_SUCCESS) {
            return ret;
        }
    }
    ...
    key_lowbit = (td_u32)(stat_cfg->key.key & 0xFFFFFFFF);
    key_highbit = (td_u32)((stat_cfg->key.key & 0xFFFFFFFF00000000) >> 32);  /* right shift 32bits */
...
static td_void isp_get_wb_stats_config(ot_vi_pipe vi_pipe, ot_isp_wb_stats_cfg *wb_cfg)
{
    ...
    wb_cfg->crop.en = ot_ext_system_awb_crop_en_read(vi_pipe);
...
static td_void isp_get_ae_stats_config(ot_vi_pipe vi_pipe, ot_isp_ae_stats_cfg *ae_cfg)
{
    ...
    ae_cfg->crop.en = ot_ext_system_ae_crop_en_read(vi_pipe);
    ...
    ae_cfg->fe_crop.en = ot_ext_system_ae_fe_crop_en_read(vi_pipe);
    ...
    for (i = 0; i < OT_ISP_AE_ZONE_ROW; i++) {
        for (j = 0; j < OT_ISP_AE_ZONE_COLUMN; j++) {
            ae_cfg->weight[i][j] = ot_ext_system_ae_weight_table_read(vi_pipe, (i * OT_ISP_AE_ZONE_COLUMN + j));
        }
    }
}
...
static td_void isp_af_stats_config_read(ot_vi_pipe vi_pipe, ot_isp_af_cfg *config)
{
    ...
    /* AF crop */
    config->crop.en = ot_ext_af_crop_enable_read(vi_pipe);
    ...
    config->fe_crop.en = ot_ext_af_fe_crop_enable_read(vi_pipe);
    ...
    config->pre_flt_cfg.en = ot_ext_af_mean_enable_read(vi_pipe);
...
static td_void isp_af_stats_h_param_iir0_read(ot_vi_pipe vi_pipe, ot_isp_af_h_param *h_param_iir0)
{
    ...
    h_param_iir0->level_depend.en = ot_ext_af_iir0_ldg_enable_read(vi_pipe);
...
static td_void isp_af_stats_h_param_iir1_read(ot_vi_pipe vi_pipe, ot_isp_af_h_param *h_param_iir1)
{
    ...
    h_param_iir1->level_depend.en       = ot_ext_af_iir1_ldg_enable_read(vi_pipe);
...
static td_void isp_af_stats_v_param_fir0_read(ot_vi_pipe vi_pipe, ot_isp_af_v_param *v_param_fir0)
{
    ...
    v_param_fir0->level_depend.en       = ot_ext_af_fir0_ldg_enable_read(vi_pipe);
...
static td_void isp_af_stats_v_param_fir1_read(ot_vi_pipe vi_pipe, ot_isp_af_v_param *v_param_fir1)
{
    ...
    v_param_fir1->level_depend.en          = ot_ext_af_fir1_ldg_enable_read(vi_pipe);
...
td_s32 ot_mpi_isp_get_stats_cfg(ot_vi_pipe vi_pipe, ot_isp_stats_cfg *stat_cfg)
{
    ...
    key_lowbit  = ot_ext_system_statistics_ctrl_lowbit_read(vi_pipe);
    key_highbit = ot_ext_system_statistics_ctrl_highbit_read(vi_pipe);
    stat_cfg->key.key = ((td_u64)key_highbit << 32) + key_lowbit;  /* left shift 32bits */
...
td_s32 ot_mpi_isp_set_pregamma_attr(ot_vi_pipe vi_pipe, const ot_isp_pregamma_attr *pregamma_attr)
{
    ...
    td_u32 i;
    isp_check_pipe_return(vi_pipe);
    isp_check_pointer_return(pregamma_attr);
...
static td_s32 isp_csc_attr_check(const ot_isp_csc_attr *csc_attr)
{
    ...
    if (csc_attr->color_gamut == OT_COLOR_GAMUT_BT2020) {
        isp_warn_trace("BT.2020 gamut is not used\n");
    }
    ...
    for (i = 0; i < OT_ISP_CSC_COEF_NUM; i++) {
        if ((csc_attr->csc_magtrx.csc_coef[i] > 4095) ||    /* 4095: max value */
            ...
            isp_err_trace("Invalid csc_coef[%d]!\n", i);
            return OT_ERR_ISP_ILLEGAL_PARAM;
        }
    }
...
td_s32 ot_mpi_isp_set_csc_attr(ot_vi_pipe vi_pipe, const ot_isp_csc_attr *csc_attr)
{
    ...
    isp_check_bool_return(csc_attr->en);
    ...
    ot_ext_system_csc_enable_write(vi_pipe, csc_attr->en);
...
td_s32 ot_mpi_isp_get_csc_attr(ot_vi_pipe vi_pipe, ot_isp_csc_attr *csc_attr)
{
    ...
    csc_attr->en            = ot_ext_system_csc_enable_read(vi_pipe);
...
static td_s32 ot_mpi_isp_set_dp_calibrate_check_para(const ot_isp_dp_static_calibrate *dp_calibrate,
...
td_s32 ot_mpi_isp_set_dp_calibrate(ot_vi_pipe vi_pipe, const ot_isp_dp_static_calibrate *dp_calibrate)
{
    ...
    if ((is_offline_mode(work_mode.running_mode) || is_striping_mode(work_mode.running_mode)) &&
        (dp_calibrate->enable_detect == TD_TRUE) && (OT_ISP_SUPPORT_OFFLINE_DPC_CALIBRATION) != 1) {
    ...
    ret = ot_mpi_isp_set_dp_calibrate_check_para(dp_calibrate, static_count_max);
    if (ret != TD_SUCCESS) {
        return ret;
    }
...
static td_s32 isp_set_dp_static_attr(ot_vi_pipe vi_pipe, const ot_isp_dp_static_attr *dp_static_attr)
{
    td_u16 i;
    td_s32 ret;
    td_u16 static_count_max = OT_ISP_STATIC_DP_COUNT_NORMAL;
...
td_s32 ot_mpi_isp_set_dp_dynamic_attr(ot_vi_pipe vi_pipe, const ot_isp_dp_dynamic_attr *dp_dynamic_attr)
{
    ...
    ret = isp_dp_dynamic_attr_check("mpi", vi_pipe, dp_dynamic_attr);
    if (ret != TD_SUCCESS) {
        return ret;
    }
    ...
    for (i = 0; i < OT_ISP_AUTO_ISO_NUM; i++) {
        ot_ext_system_dpc_dynamic_strength_table_write(vi_pipe, i, dp_dynamic_attr->auto_attr.strength[i]);
        ot_ext_system_dpc_dynamic_blend_ratio_table_write(vi_pipe, i, dp_dynamic_attr->auto_attr.blend_ratio[i]);
    }

    ot_ext_system_dpc_dynamic_manual_enable_write(vi_pipe, dp_dynamic_attr->op_type);
    ot_ext_system_dpc_dynamic_strength_write(vi_pipe, dp_dynamic_attr->manual_attr.strength);
    ot_ext_system_dpc_dynamic_blend_ratio_write(vi_pipe, dp_dynamic_attr->manual_attr.blend_ratio);

    ot_ext_system_dpc_suppress_twinkle_enable_write(vi_pipe, dp_dynamic_attr->sup_twinkle_en);
    ot_ext_system_dpc_suppress_twinkle_thr_write(vi_pipe, dp_dynamic_attr->soft_thr);
    ot_ext_system_dpc_suppress_twinkle_slope_write(vi_pipe, dp_dynamic_attr->soft_slope);
    ot_ext_system_dpc_bright_strength_write(vi_pipe, dp_dynamic_attr->bright_strength);
    ot_ext_system_dpc_dark_strength_write(vi_pipe, dp_dynamic_attr->dark_strength);
...
td_s32 ot_mpi_isp_get_dp_dynamic_attr(ot_vi_pipe vi_pipe, ot_isp_dp_dynamic_attr *dp_dynamic_attr)
{
    ...
    dp_dynamic_attr->op_type         = (ot_op_mode)ot_ext_system_dpc_dynamic_manual_enable_read(vi_pipe);
    dp_dynamic_attr->sup_twinkle_en  = ot_ext_system_dpc_suppress_twinkle_enable_read(vi_pipe);
    dp_dynamic_attr->soft_thr        = ot_ext_system_dpc_suppress_twinkle_thr_read(vi_pipe);
    dp_dynamic_attr->soft_slope      = ot_ext_system_dpc_suppress_twinkle_slope_read(vi_pipe);
    dp_dynamic_attr->bright_strength = ot_ext_system_dpc_bright_strength_read(vi_pipe);
    dp_dynamic_attr->dark_strength   = ot_ext_system_dpc_dark_strength_read(vi_pipe);

    for (i = 0; i < OT_ISP_AUTO_ISO_NUM; i++) {
        dp_dynamic_attr->auto_attr.strength[i]    = ot_ext_system_dpc_dynamic_strength_table_read(vi_pipe, i);
        dp_dynamic_attr->auto_attr.blend_ratio[i] = ot_ext_system_dpc_dynamic_blend_ratio_table_read(vi_pipe, i);
    }

    dp_dynamic_attr->manual_attr.strength    = ot_ext_system_dpc_dynamic_strength_read(vi_pipe);
    dp_dynamic_attr->manual_attr.blend_ratio = ot_ext_system_dpc_dynamic_blend_ratio_read(vi_pipe);
...
td_s32 ot_mpi_isp_set_mesh_shading_attr(ot_vi_pipe vi_pipe, const ot_isp_shading_attr *shading_attr)
{
    ...
    isp_check_bool_return(shading_attr->en);
    isp_check_bool_return(shading_attr->bnr_lsc_auto_en);
    isp_check_open_return(vi_pipe);
    isp_check_mem_init_return(vi_pipe);
    isp_check_mesh_shading_init_return(vi_pipe);
    ot_ext_system_isp_mesh_shading_enable_write(vi_pipe, shading_attr->en);
    ot_ext_system_isp_bnr_lsc_auto_enable_write(vi_pipe, shading_attr->bnr_lsc_auto_en);
...
td_s32 ot_mpi_isp_get_mesh_shading_attr(ot_vi_pipe vi_pipe, ot_isp_shading_attr *shading_attr)
{
    ...
    isp_check_mem_init_return(vi_pipe);
    isp_check_mesh_shading_init_return(vi_pipe);
    shading_attr->en            = ot_ext_system_isp_mesh_shading_enable_read(vi_pipe);
    ...
    shading_attr->bnr_lsc_auto_en = ot_ext_system_isp_bnr_lsc_auto_enable_read(vi_pipe);
...
td_s32 ot_mpi_isp_set_mesh_shading_gain_lut_attr(ot_vi_pipe vi_pipe, const ot_isp_shading_lut_attr *shading_lut_attr)
{
    ...
    for (i = 0; i < OT_ISP_RLSC_POINTS; i++) {
        ot_ext_system_isp_bnr_shading_r_gain_write(vi_pipe, i, shading_lut_attr->bnr_lsc_gain_lut.r_gain[i]);
        ot_ext_system_isp_bnr_shading_gr_gain_write(vi_pipe, i, shading_lut_attr->bnr_lsc_gain_lut.gr_gain[i]);
        ot_ext_system_isp_bnr_shading_gb_gain_write(vi_pipe, i, shading_lut_attr->bnr_lsc_gain_lut.gb_gain[i]);
        ot_ext_system_isp_bnr_shading_b_gain_write(vi_pipe, i, shading_lut_attr->bnr_lsc_gain_lut.b_gain[i]);
    }
...
td_s32 ot_mpi_isp_get_mesh_shading_gain_lut_attr(ot_vi_pipe vi_pipe, ot_isp_shading_lut_attr *shading_lut_attr)
{
    ...
    for (i = 0; i < OT_ISP_RLSC_POINTS; i++) {
        shading_lut_attr->bnr_lsc_gain_lut.r_gain[i]  = ot_ext_system_isp_bnr_shading_r_gain_read(vi_pipe, i);
        shading_lut_attr->bnr_lsc_gain_lut.gr_gain[i] = ot_ext_system_isp_bnr_shading_gr_gain_read(vi_pipe, i);
        shading_lut_attr->bnr_lsc_gain_lut.gb_gain[i] = ot_ext_system_isp_bnr_shading_gb_gain_read(vi_pipe, i);
        shading_lut_attr->bnr_lsc_gain_lut.b_gain[i]  = ot_ext_system_isp_bnr_shading_b_gain_read(vi_pipe, i);
    }
...
td_s32 ot_mpi_isp_set_auto_color_shading_attr(ot_vi_pipe vi_pipe, const ot_isp_acs_attr *acs_attr)
{
    ...
    isp_check_bool_return(acs_attr->en);
    isp_check_bool_return(acs_attr->lock_en);
    ot_ext_system_isp_acs_enable_write(vi_pipe, acs_attr->en);
    ot_ext_system_isp_acs_lock_enable_write(vi_pipe, acs_attr->lock_en);
...
td_s32 ot_mpi_isp_get_auto_color_shading_attr(ot_vi_pipe vi_pipe, ot_isp_acs_attr *acs_attr)
{
    ...
    acs_attr->en           = ot_ext_system_isp_acs_enable_read(vi_pipe);
    ...
    acs_attr->lock_en      = ot_ext_system_isp_acs_lock_enable_read(vi_pipe);
...
static td_s32 isp_mesh_shading_calibration_cfg_res_check(ot_isp_mlsc_calibration_cfg *cali_cfg)
{
    if (cali_cfg->img_width % OT_ISP_ALIGN_WIDTH != 0) {
        isp_err_trace("Err img_width:%d, width must be a muliple of 4!\n", cali_cfg->img_width);
        return OT_ERR_ISP_ILLEGAL_PARAM;
    }

    if (cali_cfg->img_height % OT_ISP_ALIGN_HEIGHT != 0) {
        isp_err_trace("Err img_height:%d, height must be a muliple of 4!\n", cali_cfg->img_height);
        return OT_ERR_ISP_ILLEGAL_PARAM;
    }

    if (cali_cfg->dst_img_width % OT_ISP_ALIGN_WIDTH != 0) {
        isp_err_trace("Err dst_img_width:%d, width must be a muliple of 4!\n", cali_cfg->dst_img_width);
        return OT_ERR_ISP_ILLEGAL_PARAM;
    }

    if (cali_cfg->dst_img_height % OT_ISP_ALIGN_HEIGHT != 0) {
        isp_err_trace("Err dst_img_height:%d, height must be a muliple of 4!\n", cali_cfg->dst_img_height);
        return OT_ERR_ISP_ILLEGAL_PARAM;
    }

    if ((cali_cfg->offset_x % 2) != 0) { /* offset must be a muliple of 2 */
        isp_err_trace("Err offset_x, bayer pattern of the cropped image does not match to the input image!\n");
        return OT_ERR_ISP_ILLEGAL_PARAM;
    }

    if ((cali_cfg->offset_y % 2) != 0) { /* offset must be a muliple of 2 */
        isp_err_trace("Err offset_y, bayer pattern of the cropped image does not match to the input image!\n");
        return OT_ERR_ISP_ILLEGAL_PARAM;
    }
...
static td_s32 isp_mesh_shading_calibration_cfg_check(ot_isp_mlsc_calibration_cfg *cali_cfg)
{
    td_s32 ret;
    ...
    if (cali_cfg->mesh_scale > 0x7) {
        isp_err_trace("Err mesh_scale:%d, mesh_scale must be smaller than 8!\n", cali_cfg->mesh_scale);
        return OT_ERR_ISP_ILLEGAL_PARAM;
    }
    ...
    if ((cali_cfg->raw_bit >= OT_ISP_BAYER_RAW_BIT_BUTT) || (cali_cfg->raw_bit < OT_ISP_BAYER_RAW_BIT_8BIT) ||
        ...
        isp_err_trace("Err raw_bit type:%d!\n", cali_cfg->raw_bit);
        return OT_ERR_ISP_ILLEGAL_PARAM;
    }

    if (cali_cfg->bayer >= OT_ISP_BAYER_BUTT) {
        isp_err_trace("Err bayer type:%d!\n", cali_cfg->bayer);
        return OT_ERR_ISP_ILLEGAL_PARAM;
    }

    if (cali_cfg->blc_offset_b > 0xFFF) {
        isp_err_trace("Err blc(chn B) offset:%d!\n", cali_cfg->blc_offset_b);
        return OT_ERR_ISP_ILLEGAL_PARAM;
    }

    if (cali_cfg->blc_offset_gb > 0xFFF) {
        isp_err_trace("Err blc(chn Gb) offset:%d!\n", cali_cfg->blc_offset_gb);
        return OT_ERR_ISP_ILLEGAL_PARAM;
    }

    if (cali_cfg->blc_offset_gr > 0xFFF) {
        isp_err_trace("Err blc(chn Gr) offset:%d!\n", cali_cfg->blc_offset_gr);
        return OT_ERR_ISP_ILLEGAL_PARAM;
    }

    if (cali_cfg->blc_offset_r > 0xFFF) {
        isp_err_trace("Err blc(chn R) offset:%d!\n", cali_cfg->blc_offset_r);
        return OT_ERR_ISP_ILLEGAL_PARAM;
    }
...
td_s32 ot_mpi_isp_set_local_cac_attr(ot_vi_pipe vi_pipe, const ot_isp_local_cac_attr *local_cac_attr)
{
    td_u8 i;
    td_s32 ret;
    isp_check_pipe_return(vi_pipe);
    ...
    isp_check_open_return(vi_pipe);
    isp_check_mem_init_return(vi_pipe);
    isp_check_local_cac_init_return(vi_pipe);
    isp_check_bool_return(local_cac_attr->en);

    ret = isp_lcac_attr_check("mpi", local_cac_attr);
    if (ret != TD_SUCCESS) {
        return ret;
    }
    for (i = 0; i < OT_ISP_LCAC_DET_NUM; i++) {
        ot_ext_system_local_cac_r_thd_table_write(vi_pipe, i, local_cac_attr->r_detect_threshold[i]);
        ot_ext_system_local_cac_g_thd_table_write(vi_pipe, i, local_cac_attr->g_detect_threshold[i]);
        ot_ext_system_local_cac_b_thd_table_write(vi_pipe, i, local_cac_attr->b_detect_threshold[i]);
        ot_ext_system_local_cac_luma_thd_table_write(vi_pipe, i, local_cac_attr->l_detect_threshold[i]);
        ot_ext_system_local_cac_cbcr_ratio_table_write(vi_pipe, i, local_cac_attr->cb_cr_ratio[i]);
    }
    for (i = 0; i < OT_ISP_LCAC_EXP_RATIO_NUM; i++) {
        ot_ext_system_local_cac_auto_cb_str_table_write(vi_pipe, i, local_cac_attr->auto_attr.de_purple_cb_strength[i]);
        ot_ext_system_local_cac_auto_cr_str_table_write(vi_pipe, i, local_cac_attr->auto_attr.de_purple_cr_strength[i]);
    }

    ot_ext_system_local_cac_manual_mode_enable_write(vi_pipe, local_cac_attr->op_type);
    ot_ext_system_local_cac_manual_cb_str_write(vi_pipe, local_cac_attr->manual_attr.de_purple_cb_strength);
    ot_ext_system_local_cac_manual_cr_str_write(vi_pipe, local_cac_attr->manual_attr.de_purple_cr_strength);
    ot_ext_system_local_cac_enable_write(vi_pipe, local_cac_attr->en);
    ot_ext_system_local_cac_purple_var_thr_write(vi_pipe, local_cac_attr->var_threshold);
    ot_ext_system_local_cac_purple_det_range_write(vi_pipe, local_cac_attr->purple_detect_range);
    ot_ext_system_local_cac_coef_update_en_write(vi_pipe, TD_TRUE);

    return TD_SUCCESS;
}

td_s32 ot_mpi_isp_get_local_cac_attr(ot_vi_pipe vi_pipe, ot_isp_local_cac_attr *local_cac_attr)
{
    td_u8 i;
    isp_check_pipe_return(vi_pipe);
    isp_check_pointer_return(local_cac_attr);
    ...
    local_cac_attr->en               = ot_ext_system_local_cac_enable_read(vi_pipe);
    local_cac_attr->purple_detect_range  = ot_ext_system_local_cac_purple_det_range_read(vi_pipe);
    local_cac_attr->var_threshold        = ot_ext_system_local_cac_purple_var_thr_read(vi_pipe);
    local_cac_attr->op_type              = ot_ext_system_local_cac_manual_mode_enable_read(vi_pipe);
    local_cac_attr->manual_attr.de_purple_cb_strength = ot_ext_system_local_cac_manual_cb_str_read(vi_pipe);
    local_cac_attr->manual_attr.de_purple_cr_strength = ot_ext_system_local_cac_manual_cr_str_read(vi_pipe);

    for (i = 0; i < OT_ISP_LCAC_DET_NUM; i++) {
        local_cac_attr->r_detect_threshold[i]    = ot_ext_system_local_cac_r_thd_table_read(vi_pipe, i);
        local_cac_attr->g_detect_threshold[i]    = ot_ext_system_local_cac_g_thd_table_read(vi_pipe, i);
        local_cac_attr->b_detect_threshold[i]    = ot_ext_system_local_cac_b_thd_table_read(vi_pipe, i);
        local_cac_attr->l_detect_threshold[i]    = ot_ext_system_local_cac_luma_thd_table_read(vi_pipe, i);
        local_cac_attr->cb_cr_ratio[i]           = ot_ext_system_local_cac_cbcr_ratio_table_read(vi_pipe, i);
    }

    for (i = 0; i < OT_ISP_LCAC_EXP_RATIO_NUM; i++) {
        local_cac_attr->auto_attr.de_purple_cb_strength[i] = ot_ext_system_local_cac_auto_cb_str_table_read(vi_pipe, i);
        local_cac_attr->auto_attr.de_purple_cr_strength[i] = ot_ext_system_local_cac_auto_cr_str_table_read(vi_pipe, i);
    }
    return TD_SUCCESS;
}

td_s32 ot_mpi_isp_set_acac_attr(ot_vi_pipe vi_pipe, const ot_isp_acac_attr *acac_attr)
{
    td_s32 ret;
    isp_check_pipe_return(vi_pipe);
    isp_check_pointer_return(acac_attr);
    isp_check_open_return(vi_pipe);
    isp_check_mem_init_return(vi_pipe);
    isp_check_acac_init_return(vi_pipe);

    isp_check_bool_return(acac_attr->en);
    ret = isp_acac_comm_attr_check("mpi", acac_attr);
    if (ret != TD_SUCCESS) {
        return ret;
    }
    ret = isp_acac_manual_attr_check("mpi", &acac_attr->manual_attr);
    if (ret != TD_SUCCESS) {
        return ret;
    }
    ret = isp_acac_auto_attr_check("mpi", &acac_attr->auto_attr);
    if (ret != TD_SUCCESS) {
        return ret;
    }
    ot_ext_system_acac_enable_write(vi_pipe, acac_attr->en);
    ot_ext_system_acac_op_type_write(vi_pipe, acac_attr->op_type);
    ot_ext_system_acac_det_mode_write(vi_pipe, acac_attr->detect_mode);

    isp_acac_auto_attr_write(vi_pipe, &acac_attr->auto_attr);
    isp_acac_manual_attr_write(vi_pipe, &acac_attr->manual_attr);

    ot_ext_system_acac_coef_update_write(vi_pipe, TD_TRUE);
    return TD_SUCCESS;
}

td_s32 ot_mpi_isp_get_acac_attr(ot_vi_pipe vi_pipe, ot_isp_acac_attr *acac_attr)
{
    isp_check_pipe_return(vi_pipe);
    isp_check_pointer_return(acac_attr);
    isp_check_open_return(vi_pipe);
    ...
    acac_attr->detect_mode = ot_ext_system_acac_det_mode_read(vi_pipe);
    acac_attr->en = ot_ext_system_acac_enable_read(vi_pipe);
    acac_attr->op_type = ot_ext_system_acac_op_type_read(vi_pipe);

    isp_acac_manual_attr_read(vi_pipe, &acac_attr->manual_attr);
    isp_acac_auto_attr_read(vi_pipe, &acac_attr->auto_attr);
    return TD_SUCCESS;
}
...
td_s32 ot_mpi_isp_set_bayershp_attr(ot_vi_pipe vi_pipe, const ot_isp_bayershp_attr *bshp_attr)
{
    td_s32 ret;
    isp_check_pipe_return(vi_pipe);
    isp_check_pointer_return(bshp_attr);
    isp_check_open_return(vi_pipe);
    isp_check_mem_init_return(vi_pipe);
    ...
    ot_ext_system_bshp_enable_write(vi_pipe, bshp_attr->en);
...
td_s32 ot_mpi_isp_get_bayershp_attr(ot_vi_pipe vi_pipe, ot_isp_bayershp_attr *bshp_attr)
{
    ...
    bshp_attr->en      = ot_ext_system_bshp_enable_read(vi_pipe);
...
td_s32 ot_mpi_isp_set_nr_attr(ot_vi_pipe vi_pipe, const ot_isp_nr_attr *nr_attr)
{
    ...
    ret = isp_nr_comm_attr_check("mpi", nr_attr);
    if (ret != TD_SUCCESS) {
        return ret;
    }
    ...
    ret = isp_nr_tnr_attr_check("mpi", &nr_attr->tnr_cfg);
    if (ret != TD_SUCCESS) {
        return ret;
    }
    isp_nr_tnr_attr_write(vi_pipe, &nr_attr->tnr_cfg);
...
td_s32 ot_mpi_isp_get_nr_attr(ot_vi_pipe vi_pipe, ot_isp_nr_attr *nr_attr)
{
    ...
    isp_nr_tnr_attr_read(vi_pipe, &nr_attr->tnr_cfg);
...
td_s32 ot_mpi_isp_set_rgbir_attr(ot_vi_pipe vi_pipe, const ot_isp_rgbir_attr *rgbir_attr)
{
    ...
    isp_check_pipe_return(vi_pipe);
    isp_check_pointer_return(rgbir_attr);
    isp_check_open_return(vi_pipe);
    isp_check_mem_init_return(vi_pipe);
...
td_s32 ot_mpi_isp_set_crosstalk_attr(ot_vi_pipe vi_pipe, const ot_isp_cr_attr *cr_attr)
{
    ...
    ot_ext_system_ge_enable_write(vi_pipe, cr_attr->en);
...
td_s32 ot_mpi_isp_get_crosstalk_attr(ot_vi_pipe vi_pipe, ot_isp_cr_attr *cr_attr)
{
    ...
    cr_attr->en          = ot_ext_system_ge_enable_read(vi_pipe);
...
td_s32 ot_mpi_isp_set_demosaic_attr(ot_vi_pipe vi_pipe, const ot_isp_demosaic_attr *demosaic_attr)
{
    td_s32 ret;
    isp_check_pipe_return(vi_pipe);
    isp_check_pointer_return(demosaic_attr);
    isp_check_bool_return(demosaic_attr->en);
    isp_check_open_return(vi_pipe);
    isp_check_mem_init_return(vi_pipe);
...
td_s32 ot_mpi_isp_set_crb_attr(ot_vi_pipe vi_pipe, const ot_isp_crb_attr *crb_attr)
{
    td_u16 i;
    td_s32 ret;
    isp_check_pipe_return(vi_pipe);
    isp_check_pointer_return(crb_attr);
    isp_check_open_return(vi_pipe);
    isp_check_mem_init_return(vi_pipe);
    isp_check_crb_init_return(vi_pipe);

    ret = isp_crb_attr_check("mpi", crb_attr);
    if (ret != TD_SUCCESS) {
        return ret;
    }
    ot_ext_system_crb_en_write(vi_pipe, crb_attr->en);
    ot_ext_system_crb_type_write(vi_pipe, crb_attr->op_type);
    ot_ext_system_crb_manual_rgain_write(vi_pipe, crb_attr->manual_attr.r_gain_limit);
    ot_ext_system_crb_manual_bgain_write(vi_pipe, crb_attr->manual_attr.b_gain_limit);
    for (i = 0; i < OT_ISP_WDR_RATIO_NUM; i++) {
        ot_ext_system_crb_auto_rgain_write(vi_pipe, i, crb_attr->auto_attr.r_gain_limit[i]);
        ot_ext_system_crb_auto_bgain_write(vi_pipe, i, crb_attr->auto_attr.b_gain_limit[i]);
    }
    ot_ext_system_crb_coef_update_en_write(vi_pipe, TD_TRUE);
    return TD_SUCCESS;
}

td_s32 ot_mpi_isp_get_crb_attr(ot_vi_pipe vi_pipe, ot_isp_crb_attr *crb_attr)
{
    td_u16 i;
    isp_check_pipe_return(vi_pipe);
    isp_check_pointer_return(crb_attr);
    ...
    isp_check_mem_init_return(vi_pipe);
    isp_check_crb_init_return(vi_pipe);
    crb_attr->en = ot_ext_system_crb_en_read(vi_pipe);
    crb_attr->op_type = ot_ext_system_crb_type_read(vi_pipe);
    crb_attr->manual_attr.r_gain_limit = ot_ext_system_crb_manual_rgain_read(vi_pipe);
    crb_attr->manual_attr.b_gain_limit = ot_ext_system_crb_manual_bgain_read(vi_pipe);
    for (i = 0; i < OT_ISP_WDR_RATIO_NUM; i++) {
        crb_attr->auto_attr.r_gain_limit[i] = ot_ext_system_crb_auto_rgain_read(vi_pipe, i);
        crb_attr->auto_attr.b_gain_limit[i] = ot_ext_system_crb_auto_bgain_read(vi_pipe, i);
    }
    return TD_SUCCESS;
}
...
td_s32 ot_mpi_isp_set_ca_attr(ot_vi_pipe vi_pipe, const ot_isp_ca_attr *ca_attr)
{
    ...
    ot_ext_system_ca_en_write(vi_pipe, ca_attr->en);
...
td_s32 ot_mpi_isp_get_ca_attr(ot_vi_pipe vi_pipe, ot_isp_ca_attr *ca_attr)
{
    ...
    ca_attr->en       = ot_ext_system_ca_en_read(vi_pipe);
...
td_s32 ot_mpi_isp_set_black_level_attr(ot_vi_pipe vi_pipe, const ot_isp_black_level_attr *black_level)
{
    ...
    if (black_level->black_level_mode >= OT_ISP_BLACK_LEVEL_MODE_BUTT) {
...
static td_s32 ot_mpi_isp_fpn_calibrate_attr_check(ot_vi_pipe vi_pipe, const ot_isp_fpn_calibrate_attr *calibrate_attr)
{
...
static td_s32 ot_mpi_isp_fpn_correction_attr_check(ot_vi_pipe vi_pipe, const ot_isp_fpn_attr *fpn_attr)
{
    td_u8 i;
    td_u8 wdr_mode = ot_ext_system_sensor_wdr_mode_read(vi_pipe);
    if (is_3to1_wdr_mode(wdr_mode)) {
        isp_err_trace("do not support FPN correction in 3to1 WDR mode!\n");
        return OT_ERR_ISP_NOT_SUPPORT;
    }
...
td_s32 ot_mpi_isp_fpn_calibrate(ot_vi_pipe vi_pipe, ot_isp_fpn_calibrate_attr *calibrate_attr)
{
    ...
    ret = ot_mpi_isp_fpn_calibrate_attr_check(vi_pipe, calibrate_attr);
    if (ret != TD_SUCCESS) {
        return ret;
    }
...
td_s32 ot_mpi_isp_set_fpn_attr(ot_vi_pipe vi_pipe, const ot_isp_fpn_attr *fpn_attr)
{
    ...
    ret = ot_mpi_isp_fpn_correction_attr_check(vi_pipe, fpn_attr);
    if (ret != TD_SUCCESS) {
        return ret;
    }
...
td_s32 ot_mpi_isp_get_dng_color_param(ot_vi_pipe vi_pipe, ot_isp_dng_color_param *dng_color_param)
{
    ...
    dng_color_param->wb_gain1.r_gain = ot_ext_system_dng_high_wb_gain_r_read(vi_pipe);
    dng_color_param->wb_gain1.g_gain = ot_ext_system_dng_high_wb_gain_g_read(vi_pipe);
    dng_color_param->wb_gain1.b_gain = ot_ext_system_dng_high_wb_gain_b_read(vi_pipe);

    dng_color_param->wb_gain2.r_gain = ot_ext_system_dng_low_wb_gain_r_read(vi_pipe);
    dng_color_param->wb_gain2.g_gain = ot_ext_system_dng_low_wb_gain_g_read(vi_pipe);
    dng_color_param->wb_gain2.b_gain = ot_ext_system_dng_low_wb_gain_b_read(vi_pipe);

    return TD_SUCCESS;
}

td_s32 ot_mpi_isp_ir_auto_run_once(ot_vi_pipe vi_pipe, ot_isp_ir_auto_attr *ir_attr)
{
    isp_check_pipe_return(vi_pipe);
    isp_check_pointer_return(ir_attr);
    isp_check_open_return(vi_pipe);
    isp_check_mem_init_return(vi_pipe);
    isp_check_bool_return(ir_attr->en);
...
static td_s32 isp_smart_info_param_check(ot_vi_pipe vi_pipe, const ot_isp_smart_info *smart_info)
{
    td_s32 i;
    ...
    for (i = 0; i < OT_ISP_FACE_NUM; i++) {
        ...
        if ((smart_info->face_roi.face_rect[i].x > max_width) || (smart_info->face_roi.face_rect[i].y > max_height)) {
            isp_err_trace("face_rect[%d].x:%d or face_rect[%d].y:%d exceeds the maximum,max_width:%d max_height:%d!\n",
                i, smart_info->face_roi.face_rect[i].x, i, smart_info->face_roi.face_rect[i].y,
        ...
        if (smart_info->face_roi.face_rect[i].x + smart_info->face_roi.face_rect[i].width > max_width) {
            isp_err_trace("The sum of face_rect[%d].x:%d and face_rect[%d].width:%d exceeds the maximum:%d!\n",
        ...
        if (smart_info->face_roi.face_rect[i].y + smart_info->face_roi.face_rect[i].height > max_height) {
            isp_err_trace("The sum of face_rect[%d].y:%d and face_rect[%d].height :%d exceeds the maximum:%d!\n",
...
td_s32 ot_mpi_isp_set_smart_info(ot_vi_pipe vi_pipe, const ot_isp_smart_info *smart_info)
{
    ...
    for (i = 0; i < OT_ISP_PEOPLE_CLASS_MAX; i++) {
        isp_check_bool_return(smart_info->people_roi[i].enable);
        isp_check_bool_return(smart_info->people_roi[i].available);
    ...
    for (i = 0; i < OT_ISP_TUNNEL_CLASS_MAX; i++) {
        isp_check_bool_return(smart_info->tunnel_roi[i].enable);
        isp_check_bool_return(smart_info->tunnel_roi[i].available);
    ...
    isp_check_bool_return(smart_info->face_roi.enable);
    isp_check_bool_return(smart_info->face_roi.available);
...
td_s32 ot_mpi_isp_calc_flicker_type(ot_vi_pipe vi_pipe, ot_isp_calc_flicker_input *input_param,
                                    ...
                                    td_u32 array_size)
{
    return isp_calc_flicker_type(vi_pipe, input_param, output_param, frame, array_size);
}
...
{
    isp_check_pipe_return(vi_pipe);
    isp_check_pointer_return(be_frame_attr);
    isp_check_open_return(vi_pipe);
    isp_check_mem_init_return(vi_pipe);

    if (be_frame_attr->frame_pos >= OT_ISP_DUMP_FRAME_POS_BUTT) {
...
td_s32 ot_mpi_isp_get_noise_calibration(ot_vi_pipe vi_pipe, ot_isp_noise_calibration *noise_calibration)
{
    ...
    isp_check_pipe_return(vi_pipe);
    isp_check_pointer_return(noise_calibration);
    isp_check_open_return(vi_pipe);
    isp_check_mem_init_return(vi_pipe);
