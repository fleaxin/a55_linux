 ...
 * use the id 0 & 1.
 */
#define sample_awb_get_extreg_id(handle)   (((handle) == 0) ? 0x4 : 0x5)

#define sample_awb_get_ctx(handle)           (&g_sample_awb_ctx[handle])
...
td_s32 sample_awb_isp_regs_init(td_s32 handle)
{
    ...
    for (i = 0; i < OT_ISP_BAYER_CHN_NUM; i++) {
        awb_ctx->awb_result.white_balance_gain[i] =
            awb_ctx->sns_dft.gain_offset[i] << 8;  /* left shift 8 for result awb gain */
    }
...
td_s32 ot_mpi_awb_register(td_s32 vi_pipe, ot_isp_3a_alg_lib *awb_lib)
{
    ot_isp_awb_register awb_register;
    td_s32 ret;

    sample_awb_check_dev_return(vi_pipe);
...
td_s32 ot_mpi_awb_unregister(td_s32 vi_pipe, ot_isp_3a_alg_lib *awb_lib)
{
    td_s32 ret;
...
td_s32 ot_mpi_awb_sensor_unreg_callback(ot_vi_pipe vi_pipe, ot_isp_3a_alg_lib *awb_lib, ot_sensor_id sensor_id)
{
    sample_awb_ctx_s *awb_ctx = TD_NULL;
    td_s32  handle;
    ...
    awb_ctx->sns_attr_info.sensor_id = 0;
